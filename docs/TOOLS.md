...
- **Inputs** — Standalone: a CSV of proteins (+ optional ligand SMILES / ion CCD codes); see `--protein_id_column_names` / `--protein_sequence_column_names` / `--ligand_*` / `--ion_*`. Via the orchestrator: just the gen FASTA — the fan-out wrapper (`scripts/run_alphafold_fanout.sh`) converts it to a CSV. **Co-fold** the class-I TPS active site for a HOLO prediction with `--af3_cofold`: `none` (default, apo protein only), `mg` (the trinuclear Mg²⁺ cluster — 3× CCD `MG`, ligated by DDXXD + NSE/DTE), or `mg_ppi` (the cluster + a diphosphate head group, CCD `POP`/pyrophosphate²⁻, a substrate-agnostic stand-in for the prenyl-PP diphosphate that bridges the cluster). Output filenames stay `<ID>.pdb` regardless. **Caveat:** AF3 free-ion placement is a *hypothesis*, not ground truth — verify the Mg land at the DDXXD/NSE side chains before trusting the holo geometry (Christianson 2017; AF3 co-folding literature).
