...
- **Env + source** — `tps_eval`; [`src/tps_eval/sequence_metrics/motif_pair_distance.py`](../src/tps_eval/sequence_metrics/motif_pair_distance.py).
...
- **Env + source** — `tps_eval`; [`src/tps_eval/esm/extract_embeddings.py`](../src/tps_eval/esm/extract_embeddings.py).
...
- **Env + source** — `tps_eval`; [`src/tps_eval/sequence_metrics/esm_pseudo_perplexity.py`](../src/tps_eval/sequence_metrics/esm_pseudo_perplexity.py).
...
- **Env + source** — `tps_eval`; [`src/tps_eval/sequence_metrics/max_sequence_identity.py`](../src/tps_eval/sequence_metrics/max_sequence_identity.py).
...
- **Env + source** — `tps_eval`; [`src/tps_eval/sequence_metrics/min_embedding_distance.py`](../src/tps_eval/sequence_metrics/min_embedding_distance.py).
...
- **External dependency** — [DIAMOND](https://github.com/bbuchfink/diamond) (Buchfink et al. 2021, *Nat. Methods*); [Swiss-Prot/UniProtKB](https://www.uniprot.org/). TPS set ([`src/tps_eval/homology_search/tps_uniprot_accessions.txt`](../src/tps_eval/homology_search/tps_uniprot_accessions.txt)) from the UniProt query `(reviewed:true) AND ((ec:4.2.3.*) OR (ec:5.5.1.*))`.
- **Env + source** — `tps_eval`; [`src/tps_eval/homology_search/swissprot_search.py`](../src/tps_eval/homology_search/swissprot_search.py).
...
- **Env + source** — `tps_eval`; [`src/tps_eval/sequence_metrics/local_sequence_search.py`](../src/tps_eval/sequence_metrics/local_sequence_search.py).

---

## Structure tools

All structure tools accept either an AlphaFold3 `af_output` tree (reads the top-ranked `<job>/<job>_model.cif`) or a flat dir of `.pdb`/`.cif`, auto-detected via the canonical loader in [`plddt.py`](../src/tps_eval/structure_metrics/plddt.py). `ID` = filename stem (= AF3 job name).
...
- **Env + source** — `tps_eval`; [`src/tps_eval/structure_metrics/plddt.py`](../src/tps_eval/structure_metrics/plddt.py).
...
- **Env + source** — `tps_eval`; [`src/tps_eval/structure_metrics/motif_structural_distance.py`](../src/tps_eval/structure_metrics/motif_structural_distance.py).
...
- **External dependency** — Biopython.
- **Env + source** — `tps_eval`; [`src/tps_eval/structure_metrics/active_site_geometry.py`](../src/tps_eval/structure_metrics/active_site_geometry.py).
...
- **External dependency** — Biopython.
- **Env + source** — `tps_eval`; [`src/tps_eval/structure_metrics/radius_of_gyration.py`](../src/tps_eval/structure_metrics/radius_of_gyration.py).
...
- **Env + source** — `pocket` (fpocket + openjdk for P2Rank's `prank`); [`src/tps_eval/structure_metrics/pocket_descriptors.py`](../src/tps_eval/structure_metrics/pocket_descriptors.py).
...
- **Env + source** — `enzyme_explorer`; [`src/tps_eval/enzyme_explorer/domain_composition.py`](../src/tps_eval/enzyme_explorer/domain_composition.py).
...
- **Env + source** — `aggrescan3d` (Py2.7 — module is Py2-compatible); [`src/tps_eval/structure_metrics/aggregation.py`](../src/tps_eval/structure_metrics/aggregation.py).
...
- **Env + source** — `tps_eval`; [`src/tps_eval/homology_search/foldseek_swissprot_search.py`](../src/tps_eval/homology_search/foldseek_swissprot_search.py).
...
- **Env + source** — `tps_eval`; [`src/tps_eval/structure_metrics/run_structural_identity.py`](../src/tps_eval/structure_metrics/run_structural_identity.py) (alignment in [`src/tps_eval/foldseek/structure_alignment.py`](../src/tps_eval/foldseek/structure_alignment.py)).
...
- **Env + source** — `enzyme_explorer_prod`; [`src/tps_eval/structure_metrics/run_domain_structural_identity.py`](../src/tps_eval/structure_metrics/run_domain_structural_identity.py) (reuses [`src/tps_eval/foldseek/domain_alignment.py`](../src/tps_eval/foldseek/domain_alignment.py)).
...
- **Env + source** — `esmfold` (`PROTEINMPNN_ENV` defaults to it); [`src/tps_eval/structure_metrics/proteinmpnn_score.py`](../src/tps_eval/structure_metrics/proteinmpnn_score.py).
...
- **Env + source** — `esmfold` (has torch + transformers + Biopython; ProteinMPNN shelled out with the same python); [`src/tps_eval/structure_metrics/self_consistency.py`](../src/tps_eval/structure_metrics/self_consistency.py).
...
- **External dependency** — Biopython.
- **Env + source** — `tps_eval`; [`src/tps_eval/structure_metrics/aromatic_lining.py`](../src/tps_eval/structure_metrics/aromatic_lining.py).
...
- **External dependency** — Biopython.
- **Env + source** — `tps_eval`; [`src/tps_eval/structure_metrics/diphosphate_sensor.py`](../src/tps_eval/structure_metrics/diphosphate_sensor.py).
...
- **External dependency** — Biopython.
- **Env + source** — `tps_eval`; [`src/tps_eval/structure_metrics/ion_site_check.py`](../src/tps_eval/structure_metrics/ion_site_check.py). For the trinuclear Mg²⁺ cluster geometry see Christianson, D. W. *Chem. Rev.* **2017**, *117*, 11570–11648.
...
- **External dependency** — Biopython.
- **Env + source** — `tps_eval`; [`src/tps_eval/structure_metrics/substrate_positioning.py`](../src/tps_eval/structure_metrics/substrate_positioning.py). Substrate SMILES in [`src/tps_eval/alphafold/cofold_substrates.py`](../src/tps_eval/alphafold/cofold_substrates.py). Christianson, D. W. *Chem. Rev.* **2017**, *117*, 11570–11648.
...
- **External dependency** — Biopython.
- **Env + source** — `tps_eval`; [`src/tps_eval/structure_metrics/cyclization_geometry.py`](../src/tps_eval/structure_metrics/cyclization_geometry.py). Christianson, D. W. *Chem. Rev.* **2017**, *117*, 11570–11648.
...
- **Env + source** — `tps_eval`; [`src/tps_eval/structure_metrics/global_confidence.py`](../src/tps_eval/structure_metrics/global_confidence.py). PAE/pTM retention lives in [`src/tps_eval/esmfold/esmfold.py`](../src/tps_eval/esmfold/esmfold.py) + [`src/tps_eval/alphafold/extract_pae.py`](../src/tps_eval/alphafold/extract_pae.py).
...
- **Env + source** — `enzyme_explorer_prod`; [`src/tps_eval/structure_metrics/interdomain_pae.py`](../src/tps_eval/structure_metrics/interdomain_pae.py).
...
- **Env + source** — `enzyme_explorer` (run from the EE `scripts/` dir so `data/` resolves to the production bundle); [`src/tps_eval/enzyme_explorer/extract_ee_domain_features.py`](../src/tps_eval/enzyme_explorer/extract_ee_domain_features.py).
...
- **Env + source** — `enzyme_explorer` (run from the EE `scripts/` dir so `data/plm_checkpoints/...` resolves); [`src/tps_eval/enzyme_explorer/extract_ee_esm1v_embeddings.py`](../src/tps_eval/enzyme_explorer/extract_ee_esm1v_embeddings.py).
...
- **Env + source** — `tps_eval`; [`src/tps_eval/specificity/extract_active_site_features.py`](../src/tps_eval/specificity/extract_active_site_features.py) (logic) + [`run_extract_active_site_features.py`](../src/tps_eval/specificity/run_extract_active_site_features.py) (argv + MARTS-DB class merge).
...
- **Output** — Plot images written to `--save_dir` (a `plots/` dir beside the gen FASTA in the orchestrator). Numeric metrics → boxplot + density; categorical/boolean metrics (motif presence, `domain_architecture`, `*_top_is_tps`) → count plots. Targets and their CSV sources are mapped in `src/tps_eval/plot/constants.py`.
...
- **Env + source** — `tps_eval`; [`src/tps_eval/plot/run_plots.py`](../src/tps_eval/plot/run_plots.py) / [`plot_comparison.py`](../src/tps_eval/plot/plot_comparison.py); targets in [`src/tps_eval/plot/constants.py`](../src/tps_eval/plot/constants.py).
...
- **Env + source** — `tps_eval`; [`scripts/run_visualization.py`](../scripts/run_visualization.py) (runner) over [`src/tps_eval/visualization/`](../src/tps_eval/visualization/) (`dimensionality_reduction.py`, `landscape_map.py`, `palette.py`).
...
- **External dependency** — Biopython.
- **Env + source** — `tps_eval`; [`src/tps_eval/specificity/sdr_divergence.py`](../src/tps_eval/specificity/sdr_divergence.py).
...
- **Env + source** — `tps_eval`; [`src/tps_eval/knn/run_substrate_class.py`](../src/tps_eval/knn/run_substrate_class.py) (logic in [`src/tps_eval/knn/substrate_class.py`](../src/tps_eval/knn/substrate_class.py)).
...
- **Output** — `src/tps_eval/reference_stats/marts_db_metric_stats.json` (committable). Per metric/column: numeric columns get count/mean/std/min/percentiles (p1…p99)/max; categorical/boolean columns get a frequency table. Included metrics: `motif_pair_distance`, `esm_pseudo_perplexity`, `plddt`, `motif_structural_distance`, `active_site_geometry`, `aggregation`, `domain_composition`, `proteinmpnn_score`, `radius_of_gyration`. Excluded (inherently comparative): identity / embedding / structural distance / broad searches / scRMSD.
- **Method** — Submits the existing per-tool job scripts on the staged MARTS-DB inputs, then chains [`aggregate_reference_stats.py`](../src/tps_eval/reference_stats/aggregate_reference_stats.py) (column-type-driven, metric-agnostic) as a dependent job.
- **External dependency** — [MARTS-DB](https://marts-db.org/) (the natural-TPS reference set).
- **Env + source** — `tps_eval` (aggregator); [`scripts/compute_reference_stats.sh`](../scripts/compute_reference_stats.sh) + [`src/tps_eval/reference_stats/aggregate_reference_stats.py`](../src/tps_eval/reference_stats/aggregate_reference_stats.py).

## Selection & funnel

The **selection layer** (`src/tps_eval/selection/`) narrows a pool of designs using the merged metric
...
- **Env + source** — `tps_eval`; [`src/tps_eval/selection/merge.py`](../src/tps_eval/selection/merge.py).
...
- **Env + source** — `tps_eval`; [`src/tps_eval/selection/select_designs.py`](../src/tps_eval/selection/select_designs.py) (+ `gate.py`/`band_filter.py`/`score.py`/`diversity_dedup.py`).
...
- **Inputs/usage** — `python src/tps_eval/selection/export_bands.py --ref_stats marts_db_esmfold_metric_stats.json --output bands.json --metrics catalytic_pocket_volume pocket_depth:p25,none [--by domain_architecture] [--default_lo p25 --default_hi p75]`. Per-metric `metric:lo,hi` overrides the default percentile edges (`none` = one-sided).
- **Env + source** — `tps_eval`; [`src/tps_eval/selection/export_bands.py`](../src/tps_eval/selection/export_bands.py).
